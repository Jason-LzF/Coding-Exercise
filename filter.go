...
// three date format used for date parsing
...
var timeOfDate timeStruct // timeOfDate store time converted from Date_reported of vulnerabilities
...
// dateToTime converts dates into time and store them into the timeOfDate structure with a slice.
// On every request the program can use the time stored in the timeOfDate straightforward for
// date comparison without converting them before comparison each time.
...
func (vul vulHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	// counter for the number of each correct parameter
	sevFound := 0
	limFound := 0
	datFound := 0
	...
	var invalid int
	// refDate consisits of three different valid input formats of date
	// Since the real input format of date is unknown, here three usual
...
// it will return a function which only checks if the Severity of a vulnerability is greater or
// equal to severity_at_least without checking any other parameters.
